...
int main(int argc, char **argv)
{
	...
	if (taskindex == 0)
	{
		...
		for (int i = 0; i < sim.num_lightcone; i++)
		{
			...
			cout << "    opening half-angle = " << sim.lightcone[i].opening * 180. / M_PI << " degrees" << endl;
	...
	for (int i = 0; i < sim.num_lightcone; i++)
	{
		if (use_lightcone[i])
		{
			...
			while (true)
			{
				...
				cinfo.opening[0] = (sim.lightcone[i].distance[0] <= cinfo.d_01 || sim.lightcone[i].distance[1] > cinfo.d_01 || cinfo.d_01 <= 0) ? 2. : cos(sim.lightcone[i].opening);
				cinfo.opening[1] = (sim.lightcone[i].distance[0] <= cinfo.d_12 || sim.lightcone[i].distance[1] > cinfo.d_12 || cinfo.d_12 <= 0) ? 2. : cos(sim.lightcone[i].opening);
				cinfo.opening[2] = (sim.lightcone[i].distance[0] <= cinfo.d_out || sim.lightcone[i].distance[1] > cinfo.d_out || cinfo.d_out <= 0) ? 2. : cos(sim.lightcone[i].opening);
...
#pragma omp parallel for
	...
	else
	{
		...
		for (int n = 0; n < Nsource; n++)
		{
			if((c = fscanf(infile, " %lf %lf %lf %*f %*e %*e %*e", source_direction+(2*n), source_direction+(1+2*n), dummy_read)) != 3)
			{
				cout << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": could not read source data from catalog for source number " << n << ", number of items read = " << c << endl;
				return -1;
			}
...
#pragma omp parallel for private(phi,phiprime,gradphi,tidalphi,ngradphi,nntidalphi,eetidalphi,exp2phi,v,tmp,dist,source_dist,dn_dtau,dlnk0_dtau,filename,outfile), reduction(+:num_live), reduction(min:min_dist), reduction(max:max_dist)
			for (int n = 0; n < Nsource; n++)
			{
				...
				if (!interpolation(&phi_prev, &phi_next, weight, dist, dr, v, &phi) || !time_derivative(&phi_prev, &phi_next, cycle_next.tau - cycle_prev.tau, dist, dr, v, &phiprime) || !gradient(&phi_prev, &phi_next, weight, dist, dr, v, gradphi) || !tidal_matrix(&phi_prev, &phi_next, weight, dist, dr, v, tidalphi))
				{
				...
				for (int i = 0; i < 3; i++)
					tmp[i] = (tmp[i] * ngradphi - gradphi[i]) * 2 * exp2phi;

				counterrotate_vector(tmp, dn_dtau, coordsys);

				dlnk0_dtau = -4. * ngradphi * exp2phi - 2. * phiprime;
...
#pragma omp critical(evala)
				{
					...
					fprintf(outfile, " %f  %f  %f  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e\n", photon[n].pos[0], photon[n].pos[1], photon[n].pos[2], exp(photon[n].lnk0) / gsl_spline_eval (aspline, tau, acc) / gsl_spline_eval (aspline, tau, acc), phi, photon[n].DA * gsl_spline_eval (aspline, tau, acc), photon[n].sigma[0], photon[n].sigma[1], photon[n].ellip[0], photon[n].ellip[1], tmp[0], tmp[1], tmp[2], gradphi[0], gradphi[1], gradphi[2], photon[n].n[0]*photon[n].n[0] + photon[n].n[1]*photon[n].n[1] + photon[n].n[2]*photon[n].n[2] - 1.);
					fclose(outfile);
				}
				...
				{
					photon[n].sigma[0] = (photon[n].sigma[0] - 0.5 * integration_step * photon[n].DA * photon[n].DA * eetidalphi[0]) / (1. + 0.5 * integration_step * dlnk0_dtau);
					photon[n].sigma[1] = (photon[n].sigma[1] - 0.5 * integration_step * photon[n].DA * photon[n].DA * eetidalphi[1]) / (1. + 0.5 * integration_step * dlnk0_dtau);

					photon[n].DAprime = (photon[n].DAprime + integration_step * photon[n].DA * (nntidalphi - tidalphi[0] - tidalphi[3] - tidalphi[5]) - integration_step * (photon[n].sigma[0]*photon[n].sigma[0] + photon[n].sigma[1]*photon[n].sigma[1]) / photon[n].DA / photon[n].DA / photon[n].DA) / (1. + 0.5 * integration_step * dlnk0_dtau);
				}

				photon[n].sigma[0] = photon[n].sigma[0] * (1. - 0.5 * integration_step * dlnk0_dtau) - 0.5 * integration_step * photon[n].DA * photon[n].DA * eetidalphi[0];
				photon[n].sigma[1] = photon[n].sigma[1] * (1. - 0.5 * integration_step * dlnk0_dtau) - 0.5 * integration_step * photon[n].DA * photon[n].DA * eetidalphi[1];
				...
				tmp[0] = photon[n].ellip[0] + integration_step * photon[n].sigma[0] * sqrt(tmp[2] + 4.) / photon[n].DA / photon[n].DA;
				tmp[1] = photon[n].ellip[1] + integration_step * photon[n].sigma[1] * sqrt(tmp[2] + 4.) / photon[n].DA / photon[n].DA;
				tmp[2] = tmp[0]*tmp[0] + tmp[1]*tmp[1];

				photon[n].ellip[0] += 2. * integration_step * photon[n].sigma[0] * sqrt(tmp[2] + 4.) / photon[n].DA / photon[n].DA;
				photon[n].ellip[1] += 2. * integration_step * photon[n].sigma[1] * sqrt(tmp[2] + 4.) / photon[n].DA / photon[n].DA;
				...
				for (int i = 0; i < 3; i++)
				{
					...
					photon[n].pos[i] += integration_step * photon[n].n[i] * exp2phi;
				}
				...
				if (stepcount % 1000 == 0)
				{
					...
					for (int i = 0; i < 3; i++) photon[n].n[i] /= tmp[0];
				}
				...
				if (dist > source_dist)
				{
					...
					for (int i = 0; i < 3; i++)
						photon[n].pos[i] += source_dist * photon[n].n[i] * exp2phi;

					photon[n].lnk0 += source_dist * dlnk0_dtau;

					photon[n].DA += source_dist * photon[n].DAprime;

					photon[n].ellip[0] += 2. * source_dist * photon[n].sigma[0] * sqrt(tmp[2] + 4.) / photon[n].DA / photon[n].DA;
					photon[n].ellip[1] += 2. * source_dist * photon[n].sigma[1] * sqrt(tmp[2] + 4.) / photon[n].DA / photon[n].DA;
...
#pragma omp critical(evala)
{
		...
		cout << " task " << taskindex << " maximum shooting angle correction was " << sqrt(maxcorr) << " radians." << endl << endl;
		...
		else
		{
			...
			fprintf(outfile, "# theta      phi        1+z_obs    D_A           Re(e)          Im(e)           residual\n");
			for (int n = 0; n < Nsource; n++)
			{
				if (photon[n].stop > 0)
					fprintf(outfile, "  %f   %f   %f   %f   %e   %e   %e\n", source_direction[2*n], (source_direction[1+2*n] < 0.) ? source_direction[1+2*n] + 2.*M_PI : source_direction[1+2*n], photon[n].lnk0, photon[n].DA, photon[n].ellip[0], photon[n].ellip[1], sqrt((photon[n].pos[0]-source_pos[3*n])*(photon[n].pos[0]-source_pos[3*n]) + (photon[n].pos[1]-source_pos[1+3*n])*(photon[n].pos[1]-source_pos[1+3*n]) + (photon[n].pos[2]-source_pos[2+3*n])*(photon[n].pos[2]-source_pos[2+3*n]))*sim.numpts);
				else
					fprintf(outfile, "  %f   %f   -1.00000   -1.00000   0.000000e+00   0.000000e+00   -1.000000e+00\n", source_direction[2*n], (source_direction[1+2*n] < 0.) ? source_direction[1+2*n] + 2.*M_PI : source_direction[1+2*n]);
			}
			fclose(outfile);
		}
...
bool gradient(metric_container * field_prev, metric_container * field_next, const double weight, const double dist, const double dr, double * v, double * result, int debug)
{
	...
	if (inner_prev->second.hdr.distance == 0)
	{
		...
		Npix = nside2npix(outer_prev->second.hdr.Nside);
...
// 0.585401227586727; // (sin(acos(2./3.)) * MP_I / 4.)
		...
		else // Nside > 2
		{
			...
			grad[0] = linear_interpolation(inner_prev->second.pixel[3*i+13] - inner_prev->second.pixel[i], inner_next->second.pixel[3*i+13] - inner_next->second.pixel[i], weight) / inner_prev->second.hdr.distance / (sqrt(6.66666666666666 - sqrt(16. + (4. / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside - 26.6666666666666) / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside) - 2. / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside) / inner_prev->second.hdr.Nside);
// (acos(1. - 3./(inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside)) - acos(1. - 1./(3. * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside)));
			...
			d = sqrt(6. * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside - 4.); // sin(theta)

			grad[1] = linear_interpolation(inner_prev->second.pixel[2*i+5] - inner_prev->second.pixel[2*i+4], inner_next->second.pixel[2*i+5] - inner_next->second.pixel[2*i+4], weight) / inner_prev->second.hdr.distance / (d * 0.5102445764867864 / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside);

			//rotate

			result[0] = ((i % 2) ? -0.471404520791032 : 0.471404520791032) * (d * grad[2] + (1.5 * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside - 2.) * grad[0]) / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside - 0.707106781186547 * grad[1];
			result[1] = 0.471404520791032 * (d * grad[2] + (1.5 * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside - 2.) * grad[0]) / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside + ((i % 2) ? -0.707106781186547 : 0.707106781186547) * grad[1];
			...
			result[2] = ((1.5 * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside - 2.) * grad[2] - d * grad[0]) / 1.5 / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside;

			//return true;
		}
	}
	else if (i < (int64_t) inner_prev->second.hdr.Nside * ((int64_t) inner_prev->second.hdr.Nside - 1) * 2) // in north polar cap
	{
#ifdef DEBUG
		...
		grad[0] /= sqrt(12. * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside * (ring*ring+1) - 2. * (ring*ring-1) * (ring*ring - 1 + sqrt(((2.44948974278318*inner_prev->second.hdr.Nside + 1.)*(2.44948974278318*inner_prev->second.hdr.Nside + 1.) - ring*ring)*((2.44948974278318*inner_prev->second.hdr.Nside - 1.)*(2.44948974278318*inner_prev->second.hdr.Nside - 1.) - ring*ring)))) * inner_prev->second.hdr.distance / 3. / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside;
// (acos(1. - (ring+1) * (ring+1) / (3. * (double) inner_prev->second.hdr.Nside * (double) inner_prev->second.hdr.Nside)) - acos(1. - (ring-1) * (ring-1) / (3. * (double) inner_prev->second.hdr.Nside * (double) inner_prev->second.hdr.Nside))) * inner_prev->second.hdr.distance;
		...
		d = ring * sqrt(6. - (double) (ring*ring) / (double) inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside) / 3. / inner_prev->second.hdr.Nside; // sin(theta)

		result[0] = d * grad[2] + (1. - (ring*ring) / (3. * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside)) * grad[0];
		...
		result[1] = (d * grad[2] + (1. - (ring*ring) / (3. * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside)) * grad[0]) * result[1] + result[2] * grad[1];
		result[2] = (1. - (ring*ring) / (3. * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside)) * grad[2] - d * grad[0];

		//return true;
	}
	else if (i < inner_prev->second.hdr.Nside * (inner_prev->second.hdr.Nside + 1) * 2) // on northern circle
	{
#ifdef DEBUG
...
// (acos((2 * (int64_t) inner_prev->second.hdr.Nside - ring - 1) / (1.5 * inner_prev->second.hdr.Nside)) - acos((2 * (int64_t) inner_prev->second.hdr.Nside - ring + 1) / (1.5 * inner_prev->second.hdr.Nside)));
		...
		d = sqrt(1. - (2 * inner_prev->second.hdr.Nside - ring) * (2 * inner_prev->second.hdr.Nside - ring) / (2.25 * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside)); // sin(theta)

		grad[1] = linear_interpolation(inner_prev->second.pixel[j] - inner_prev->second.pixel[i], inner_next->second.pixel[j] - inner_next->second.pixel[i], weight) / inner_prev->second.hdr.distance / (2. * d * sin(0.25 * M_PI / inner_prev->second.hdr.Nside));
// (sin(acos((2 * (int64_t) inner_prev->second.hdr.Nside - ring) / (1.5 * inner_prev->second.hdr.Nside))) * M_PI / (2 * (int64_t) inner_prev->second.hdr.Nside));
		...
		result[0] = (d * grad[2] + ((2 * inner_prev->second.hdr.Nside - ring) / 1.5 / inner_prev->second.hdr.Nside) * grad[0]) * result[2] - result[1] * grad[1];
		result[1] = (d * grad[2] + ((2 * inner_prev->second.hdr.Nside - ring) / 1.5 / inner_prev->second.hdr.Nside) * grad[0]) * result[1] + result[2] * grad[1];
		result[2] = ((2 * inner_prev->second.hdr.Nside - ring) / 1.5 / inner_prev->second.hdr.Nside) * grad[2] - d * grad[0];
...
// (acos(1. - (ring+1) * (ring+1) / (3. * (double) inner_prev->second.hdr.Nside * (double) inner_prev->second.hdr.Nside)) - acos(1. - (ring-1) * (ring-1) / (3. * (double) inner_prev->second.hdr.Nside * (double) inner_prev->second.hdr.Nside))) * inner_prev->second.hdr.distance;
 ...
 inner_prev->second.hdr.distance / 0.797054997187545; // 2. * sin(0.5 * (acos(1./3.) - acos(11./12.))) / 0.819821555018427; // (acos(1./3.) - acos(11./12.))
		...
		else // Nside > 2
		{
			...
			grad[0] = linear_interpolation(inner_prev->second.pixel[i] - inner_prev->second.pixel[3*j+Npix-23], inner_next->second.pixel[i] - inner_next->second.pixel[3*j+Npix-23], weight) / inner_prev->second.hdr.distance / (sqrt(6.66666666666666 - sqrt(16. + (4. / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside - 26.6666666666666) / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside) - 2. / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside) / inner_prev->second.hdr.Nside);
// (acos(1. - 3./(inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside)) - acos(1. - 1./(3. * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside)));

			d = sqrt(6. * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside - 4.); // sin(theta)

			grad[1] = linear_interpolation(inner_prev->second.pixel[2*j+Npix-11] - inner_prev->second.pixel[2*j+Npix-12], inner_next->second.pixel[2*j+Npix-11] - inner_next->second.pixel[2*j+Npix-12], weight) / inner_prev->second.hdr.distance / (d * 0.5102445764867864 / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside);
// (sin(acos(1. - 4. / (3. * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside))) * M_PI / 4.);
			...
			result[2] = ((2. - 1.5 * inner_prev->second.hdr.Nside * inner_prev->second.hdr.Nside) * grad[2] - d * grad[0]) / 1.5 / inner_prev->second.hdr.Nside / inner_prev->second.hdr.Nside;
...
bool tidal_matrix(metric_container * field_prev, metric_container * field_next, const double weight, const double dist, const double dr, double * v, double * result, int debug)
{
	...
	else if (j < (int64_t) center_prev->second.hdr.Nside * ((int64_t) center_prev->second.hdr.Nside - 1) * 2) // in north polar cap
	{
		...
		tidal[3] /= 1. - ((2 * ring * (ring-1) + 1) / 6. - ring * (ring-1) * (ring * (ring-1) + sqrt((6. * center_prev->second.hdr.Nside * center_prev->second.hdr.Nside - (ring-1) * (ring-1)) * (6. * center_prev->second.hdr.Nside * center_prev->second.hdr.Nside - ring * ring))) / center_prev->second.hdr.Nside / center_prev->second.hdr.Nside) / center_prev->second.hdr.Nside / center_prev->second.hdr.Nside;
