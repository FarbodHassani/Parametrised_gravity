...
double pointInShell(double * pos, lightcone_geometry & lightcone, double & outer, double & inner, double * vertex)
{
	...
	{
		if (acos(((pos[0]-vertex[0])*lightcone.direction[0] + (pos[1]-vertex[1])*lightcone.direction[1] + (pos[2]-vertex[2])*lightcone.direction[2]) / d) < lightcone.opening) return d;
		else return -1.;
	}
	else return d;
}
...
#ifdef CHECK_LATERAL_OVERLAP
								if (i < sim.num_lightcone-1 && sim.lightcone[i].opening - acos(((pos[0] - vertexlist[tryvertex][0])*sim.lightcone[i].direction[0] + (pos[1] - vertexlist[tryvertex][1])*sim.lightcone[i].direction[1] + (pos[2] - vertexlist[tryvertex][2])*sim.lightcone[i].direction[2]) / dist) < CHECK_LATERAL_OVERLAP) IDlookup.insert(IDbatch[it]);
#endif
