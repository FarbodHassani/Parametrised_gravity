...
bool pointInShell(double * pos, lightcone_geometry & lightcone, double & outer, double & inner, double * vertex, int * wrap)
{
	...
	{
		if (acos(((pos[0]-(vertex[0]-wrap[0]))*lightcone.direction[0] + (pos[1]-(vertex[1]-wrap[1]))*lightcone.direction[1] + (pos[2]-(vertex[2]-wrap[2]))*lightcone.direction[2]) / d) < lightcone.opening) return true;
		else return false;
	}
	else return true;
}
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < sim.num_lightcone; i++)
	{
		if (use_lightcone[i])
		{
			...
			if (cos(sim.lightcone[i].opening) > -0.999) direction = sim.lightcone[i].direction;
	...
	for (int c = min_cycle; c <= max_cycle; c++)
	{
		...
		for (int i = 0; i < sim.num_lightcone; i++)
		{
			...
			for (dist = min_dist; dist <= max_dist; dist++)
			{
				...
				Npix = 4;
				for (int ring = 2; Npix < (1. - cos(sim.lightcone[i].opening)) * 6 * Nside * Nside; ring++)
				{
