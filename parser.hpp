...
int parseMetadata(parameter * & params, const int numparam, metadata & sim, cosmology & cosmo, icsettings & ic)
{
  ...
  sim.f_R_flag = 0;
	...
	if (!parseParameter(params, numparam, "fR0", cosmo.fR0))
		cosmo.fR0 = 0.;
  // if (!parseParameter(params, numparam, "b_yukawa", cosmo.b_yukawa))
  // 	cosmo.b_yukawa = 3.0;
  ...
  if (!parseParameter(params, numparam, "r_th", cosmo.r_th))
  	cosmo.r_th = 7.;
  if (!parseParameter(params, numparam, "screening", cosmo.screening))
      cosmo.screening = 1;
  if (!parseParameter(params, numparam, "w0_fld", cosmo.w0_fld))
    ...
    if (sim.f_R_flag==1)
    {
      COUT << " f(R) gravity requested the parameters are: f_R0 = " << cosmo.fR0 << ", b_chameleon= " << cosmo.b_cham << ", k_env= "<< cosmo.k_env << ", r_th= "<<cosmo.r_th << endl;
    }
	}
